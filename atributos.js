...
// ===== NOVO: Estado do sistema de pontos =====
let sistemaPontos = {
  pontosIniciais: 100,
...
// ===== NOVO: Função para sincronizar com Dashboard =====
function sincronizarComDashboard() {
  try {
    // Calcula custos dos atributos atuais
    const custoST = (personagemAtributos.ST - 10) * 10;
    const custoDX = (personagemAtributos.DX - 10) * 20;
    const custoIQ = (personagemAtributos.IQ - 10) * 20;
    const custoHT = (personagemAtributos.HT - 10) * 10;
    const totalAtributos = custoST + custoDX + custoIQ + custoHT;
    
    // Atualiza pontos gastos em atributos
    sistemaPontos.pontosGastos.atributos = totalAtributos;
    ...
    const dadosParaDashboard = {
      ...
      pontosGastosAtributos: totalAtributos,
      pontosGastosTotais: Object.values(sistemaPontos.pontosGastos).reduce((a, b) => a + b, 0),
      sistemaPontos: sistemaPontos,
      bonus: personagemAtributos.bonus
    };
    
    localStorage.setItem('gurps_atributos_dashboard', JSON.stringify(dadosParaDashboard));
    
    // Dispara evento customizado para o dashboard
    const evento = new CustomEvent('atributosAtualizados', {
      detail: dadosParaDashboard
    });
    window.dispatchEvent(evento);
    
  } catch (error) {
    console.warn('Erro ao sincronizar com dashboard:', error);
  }
}

// ===== FUNÇÕES PRINCIPAIS =====

// Função para alterar atributos principais
function alterarAtributo(atributo, valor) {
  ...
  atualizarTudo();
  sincronizarComDashboard(); // NOVO: Sincroniza com dashboard
}
...
function calcularCustos() {
  ...
  // NOVO: Atualiza sistema de pontos
  sistemaPontos.pontosGastos.atributos = totalGastos;
}
...
function salvarLocal() {
  try {
    const dados = {
      ...
      bonus: personagemAtributos.bonus,
      sistemaPontos: sistemaPontos // NOVO: Salva sistema de pontos também
    };
    localStorage.setItem('gurps_atributos', JSON.stringify(dados));
    
    // NOVO: Salva também separadamente para o dashboard
...
function carregarLocal() {
  try {
    ...
    if (dadosSalvos) {
      ...
      if (dados.atributos) {
        personagemAtributos.ST = dados.atributos.ST || 10;
        personagemAtributos.DX = dados.atributos.DX || 10;
        personagemAtributos.IQ = dados.atributos.IQ || 10;
        personagemAtributos.HT = dados.atributos.HT || 10;
      ...
      if (dados.bonus) {
        ...
        ['PV', 'PF', 'Vontade', 'Percepcao', 'Deslocamento'].forEach(atributo => {
          const input = document.getElementById('bonus' + atributo);
          if (input) {
            const valor = personagemAtributos.bonus[atributo] || 0;
            input.value = valor;
      ...
      // NOVO: Carrega sistema de pontos se existir
      if (dados.sistemaPontos) {
        sistemaPontos = dados.sistemaPontos;
      }
      
      return true;
    }
  } catch (error) {
    console.warn('Não foi possível carregar:', error);
  }
  return false;
}

// ===== NOVO: Função para obter dados dos atributos =====
function obterDadosAtributos() {
  return {
    ...
    HT: personagemAtributos.HT,
    bonus: personagemAtributos.bonus,
    pontosGastosAtributos: sistemaPontos.pontosGastos.atributos
  };
}

// ===== NOVO: Função para atualizar pontos do dashboard =====
function atualizarPontosDashboard(novosPontos) {
  if (novosPontos) {
    sistemaPontos = { ...sistemaPontos, ...novosPontos };
    sincronizarComDashboard();
  }
}
...
function inicializarAtributos() {
  ...
  // Carrega dados salvos ou inicia com padrão
  if (!carregarLocal()) {
    personagemAtributos = {
      ST: 10,
      DX: 10,
    ...
    personagemAtributos.IQ = 10;
  }
  
  // Executa primeira atualização
  atualizarTudo();
  
  // NOVO: Ouvinte para eventos do dashboard
  window.addEventListener('dashboardAtualizado', function(event) {
    if (event.detail && event.detail.sistemaPontos) {
      atualizarPontosDashboard(event.detail.sistemaPontos);
    }
  });
...
window.obterDadosAtributos = obterDadosAtributos; // NOVO: Exporta função
window.atualizarPontosDashboard = atualizarPontosDashboard; // NOVO: Exporta função
